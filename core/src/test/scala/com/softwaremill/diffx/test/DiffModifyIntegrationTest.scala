...
class DiffModifyIntegrationTest extends AnyFlatSpec with Matchers {
  ...
  it should "allow importing and exporting implicits" in {
    implicit val d: Diff[Person] = Diff[Person].ignore(_.name)
  ...
  it should "ignore only on right" in {
    ...
    implicit val wrapperDiff: Diff[Wrapper] = Diff[Wrapper].ignore(_.e.eachRight.name)
  ...
  it should "ignore only on left" in {
    ...
    implicit val wrapperDiff: Diff[Wrapper] = Diff[Wrapper].ignore(_.e.eachLeft.name)
  ...
  it should "match map entries by values" in {
    implicit val lookupDiff: Diff[MyLookup] = Diff[MyLookup]
      .modify(_.map)
      .useMatcher(
  ...
  it should "use overrided object matcher when comparing set" in {
    implicit val lookupDiff: Diff[Startup] = Diff[Startup]
      .modify(_.workers)
